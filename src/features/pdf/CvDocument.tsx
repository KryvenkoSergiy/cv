...
// Deeper than the site's emerald: the PDF is dark text on white paper,
// and #047857 keeps accent text at ~5.5:1 there.
const ACCENT = "#047857";
const NAVY = "#0f0f2e";
const GRAY = "#6b7280";
...
const IconGlobe = () => (
  <Svg width={10} height={10} viewBox="0 0 24 24">
    <Circle cx="12" cy="12" r="10" fill="none" stroke={ACCENT} strokeWidth="1.8" />
    <Path
...
const s = StyleSheet.create({
  ...
  // Takes the free width, so a long title wraps instead of running into the links.
  headerMain: {
    flex: 1,
    paddingRight: 16,
  },
  ...
  // Education and Languages share one row: both are short, and stacked they
  // pushed Contact onto a third page.
  sectionRow: {
    flexDirection: "row",
    gap: 24,
  },
...
// Email, LinkedIn and the portfolio are already in the header.
const contactItems = [
    ...
    <Page size='A4' style={s.page}>
      ...
      <View style={s.header}>
        <View style={s.headerMain}>
          ...
          <Text style={s.specialty}>{personalInfo.specialty}</Text>
        </View>
      ...
      <View style={s.section}>
        ...
        {earlierExperiences.length > 0 && (
          <Text style={s.earlierNote}>
            Earlier roles ({earlierExperiences.at(-1)!.startDate} - {earlierExperiences[0].endDate}):{" "}
            {earlierExperiences.map(e => e.client ?? e.company).join(", ")}. Full timeline:{" "}
            <Link src={`${personalInfo.portfolio}experience#earlier-work`} style={s.headerLink}>
              {`${personalInfo.portfolioDisplay}/experience`}
            </Link>
          </Text>
        )}
      </View>
      ...
      <View style={s.section}>
        ...
        {skillCategories.map(cat => (
          <Text key={cat.category} style={s.skillFlatRow}>
      ...
      <View style={[s.section, s.sectionRow]} wrap={false}>
        {/* Education */}
        <View style={[s.sectionCol, { flex: 2 }]}>
        ...
        {/* Languages — ATS parsers look for this heading specifically. */}
        <View style={s.sectionCol}>
          <Text style={s.sectionTitle}>Languages</Text>
          {spokenLanguages.map(l => (
            <Text key={l.language} style={s.skillFlatRow}>
              <Text style={s.skillFlatCat}>{l.language}: </Text>
              {l.level}
            </Text>
          ))}
        </View>
      ...
      {/* Contact */}
      <View style={s.section}>
        <Text style={s.sectionTitle} minPresenceAhead={40}>Contact</Text>
      ...
      <View style={s.footer} fixed>
        ...
        {/* `fixed` must sit on this Text, not just the parent View — the
            render callback only re-evaluates per page on fixed elements,
            otherwise every page prints "1 / 1". */}
        <Text
          style={s.footerText}
          fixed
